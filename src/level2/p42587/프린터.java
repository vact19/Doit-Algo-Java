...
public class 프린터 {
    ...
    public static int solution(int[] priorities, int location) {
        ...
        while (true){
            ...
            System.out.println(polled + " 을 꺼냄");
            if (isMaxPriority(intQueue, polled)){ // 중요도가 단독 혹은 공동 1위일 때
                // 인쇄한다. 그런데 처음 낙인 찍어둔 그 숫자면 반복문 탈출한다.
                count++;
                System.out.println(polled + "를 출력함. " + count + "번째 출력");
                if (polled / 10 >= 1)
                    return count;
            } else { // 우선순위가 밀릴 때
                // front 에서 가져온 값을 다시 rear 에 넣는다.
                System.out.println(polled + "가 중요도에서 밀려서 rear로 이동");
                intQueue.offer(polled);
            }
        }
    }

    private static void printQueue(Queue<Integer> intQueue) {
        System.out.println("큐 목록 출력");
